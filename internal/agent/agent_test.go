...
func TestPreparePrompt_FiltersImageAttachments(t *testing.T) {
	...
	history, files := agent.preparePrompt(msgs, nil, false, imageAtt)
	...
	history, files = agent.preparePrompt(msgs, nil, true, imageAtt)
	require.Len(t, history, 2)
...
func TestPreparePrompt_OrphanedToolUse(t *testing.T) {
	...
	history, _ := agent.preparePrompt(msgs, nil, true)
